...
**"빛이 있으라"는 사건이 아니라 출력의 복원이다.**

중력은 구조를 만들고, 빛은 환경을 만든다.
태양은 이미 1.0 M☉의 주계열 항성으로 존재했지만,
v1.2.0까지는 광도(L) 출력이 없었다 — 중력만 있고 빛이 없는 별.
질량으로부터 광도를 유도하고, 광도로부터 조도·온도·복사압을 계산하는 순간,
태양은 비로소 "환경을 만드는 별"이 된다.
이것이 "빛이 있으라"의 엔지니어링적 의미다.

이 레이어는 항성 내부 물리(핵융합, 대류, 중심온도)를 풀지 않는다.
주계열 항성을 가정하고, 질량-광도 관계(L = M^α)로부터
외부 출력(조도, 온도, 복사압)만 계산하는 관측자 모델이다.
