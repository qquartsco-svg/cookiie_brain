...
조→모→체루빔→EdenOS 흐름에서, 이 폴더는 마지막 **운영 체제 레이어**다.

- `_01_beginnings/joe`  — JOE: 행성 거시 물리 탐사, planet_stress·instability로 붕괴 행성 걸러냄.
- `_02_creation_days/`   — MOE: day1~7, fields/physics/engines/day7/runner 가 6도메인 환경 스냅샷을 구축.
- `_03_eden_os_underworld/eden/search.py` — EdenSearchEngine: 이 스냅샷에서 EdenCandidate / SearchResult 산출.
- `_03_eden_os_underworld/eden/eden_os/cherubim_guard.py` — CherubimGuard: 에덴 Basin 접근 정책(Allow/Deny/Alert) 부여.
- `_03_eden_os_underworld/eden/eden_os/eden_os_runner.py` — EdenOSRunner: 선택된 IC를 OS 커널 위에서 실행 (관리·추방·계승 루프).

향후 `eden_os_bridge.py` 모듈이 Cherubim/EdenSearch 결과를 받아 곧바로 EdenOSRunner 인스턴스를 만드는 helper를 제공한다.
