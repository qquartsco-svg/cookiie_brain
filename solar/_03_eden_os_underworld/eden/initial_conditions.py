...
핵심 철학:
  ...
  레이어 대응 (PANGEA / FEYNMAN 조·모 비유):
    Macro (조/Joe): L0(자전·형상) + L1(물 예산) — 거시 뼈대. 본 모듈의 omega_spin, obliquity,
                    GEL_surface_eden_m, W_canopy_ref_km3 등은 L0/L1용 슬롯.
    Micro (모/Moe): L2(planet_stress, instability) + L3(Firmament, 수명) — 미시 환경.
                    H2O_canopy, UV_shield, pressure_atm 등은 L3/수명 엔진이 읽음.
...
참조:
  - docs/PANGEA_ROTATION_AND_WATER_BUDGET.md (L0~L3, 조/모 = Macro/Micro)
  - docs/FEYNMAN_VOL1_AS_PLANET_MOTION.md (§9 조·모)
...
class InitialConditions:
    ...
    레이어 (조/모 = Macro/Micro, docs/PANGEA + FEYNMAN §9):
      L0/L1 (Macro): omega_spin_rad_s, obliquity_deg, GEL_surface_eden_m, W_canopy_ref_km3
      L2/L3 (Micro): H2O_canopy, UV_shield, pressure_atm 등 → Firmament·수명 엔진이 읽음.
    ...
    # ── L0/L1 (Macro) 슬롯 — PANGEA 문서대로 확장 시 사용. None이면 기존 동작 유지.
    omega_spin_rad_s:    Optional[float] = None   # L0: 자전 각속도 [rad/s]
    obliquity_deg:       Optional[float] = None   # L0: 축 기울기 [deg]
    GEL_surface_eden_m: Optional[float] = None   # L1: 에덴 표면 GEL [m]
    W_canopy_ref_km3:   Optional[float] = None   # L1→L3 매핑용 기준 궁창 부피 [km³]
    ...
    def to_runner_kwargs(self) -> Dict:
        ...
        τ, ε_a, T_surface, pole_eq_delta, albedo 등은 Eden이 동역학으로 계산한 값.
        Day2 AtmosphereColumn은 이 값들이 넘어오면 재계산하지 않음 (더블카운트 방지).
        """
        return {
