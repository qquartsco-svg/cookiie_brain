...
**모델 스코프(중요)**: 본 레이어는 **전역 평균 0차(0D) 경계조건** 모델이다.  
즉, 아래는 *한 개의 숫자 \(A_{\mathrm{eff}}\)* 로 지표를 요약하는 근사이며, 다음 효과는 아직 포함하지 않는다.

- **해양 glint**(태양 고도각/관측각에 따른 반사율 변화)
- 위도·빙하·식생·구름 등에 따른 **공간 분포 알베도**
- 대륙 분포의 **공간성(격자)**, 동적 `land_fraction(t)`
