...
### 전자기 레이어 개념 / EM Layer Concepts

core/(중력/세차) 위에 얹는 "환경 필드" 3단계.
궤도에 힘을 되먹이지 않는 **관측자 레이어**다.

```
태양 ●━━━━━━━━━━━━━━━━━━━━━━━━● 지구
     │                            │
     │  Phase 3: 태양풍            │  Phase 2: 자기쌍극자장
     │  태양에서 뿜어나오는 것    │  지구가 스스로 만드는 것
     │  P_sw, 입자, 복사, IMF    │  B(r) ∝ 1/r³
     │  전부 1/r²로 약해짐       │  자전축에 연동
     │                            │
     └───────────┬────────────────┘
                 │
                 ▼
...
#### Phase 2 — 자기쌍극자장 (Magnetic Dipole)

지구가 자기 내부에서 만들어내는 "보이지 않는 갑옷".
...
```
코드 흐름:
  입력: spin_axis(t), 관측 위치 r       ← core/에서 읽기 전용
  처리: magnetic_axis() → B_field()     ← 쌍극자 방정식
  출력: B 벡터, 자기 위도, L-shell      ← DipoleFieldPoint
```

#### Phase 3 — 태양풍 / 복사압 (Solar Wind)

태양에서 쏟아지는 "폭풍" — 지구가 막아내야 하는 외부 공격.

태양은 끊임없이 하전 입자(양성자, 전자)와 전자기 복사를 우주로 내뿜는다.
전부 **1/r²**로 약해진다 — 구면(면적 4πr²)으로 퍼지기 때문.

| 물리량 | 기호 | 1 AU 실측값 | 역할 |
|--------|------|-------------|------|
| 동압 | P_sw | ~2.0 nPa | 자기권 경계 결정 (Phase 4 핵심 입력) |
| 입자 플럭스 | Phi | ~3×10⁸ /cm²/s | 입자 수 밀도 × 속도 |
| 복사압 | P_rad | ~4.56 μPa | 동압의 ~1/500 (거의 무시) |
...
```
코드 흐름:
  입력: 관측 위치, 태양 위치             ← core/에서 읽기 전용
  처리: 거리 계산 → 각 물리량 × (r₀/d)²  ← 1/r² 감쇠
  출력: SolarWindState (동압, 플럭스, 복사압, IMF, 풍향, 풍속)
```

#### Phase 4 — 자기권 (Magnetosphere)

갑옷(자기장)과 폭풍(태양풍)이 부딪혀서 생기는 "방어 경계선".

Phase 2(자기장)와 Phase 3(태양풍)을 동시에 놓으면,
한쪽은 밀어내고(자기장) 한쪽은 밀고 들어온다(태양풍).
두 힘이 **균형**을 이루는 지점이 자기권의 경계다.
...
```
코드 흐름:
  입력: MagneticDipole (B₀)  +  SolarWind (P_sw)  +  spin_axis, pos, radius
  처리: magnetopause_distance() → shielding() → cusp → energy_input()
...
| Phase | 감쇠 | 이유 |
|-------|------|------|
| Phase 2 자기장 | 1/r³ | 쌍극자(+극/-극 쌍)는 먼 거리에서 서로 상쇄 → 단극자보다 빠르게 감소 |
| Phase 3 태양풍 | 1/r² | 태양에서 나온 에너지가 구면(4πr²)으로 균일 확산 → 면적당 세기 감소 |
...
**EM / cognitive 레이어는 관측자(observer) 모드로 동작한다:**
...
| radiation_ratio | 0.002 | 이론값 | 복사압/동압 비율 (1 AU) |
