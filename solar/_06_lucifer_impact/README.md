...
## 3. 완전 독립 모듈 (Lucifer Engine)

동일한 충돌 추정 로직을 **CookiieBrain 무의존** 단일 패키지로 쓸 수 있다.

- **위치**: `ENGINE_HUB/00_PLANET_LAYER/Lucifer_Engine/`
- **패키지명**: `lucifer_engine` (`pip install -e .` 후 `from lucifer_engine import estimate_impact`)
- **구성**: `lucifer_engine/` 하위에 `__init__.py`, `impact_estimator.py`, `__main__.py` + 루트에 README, pyproject.toml, SIGNATURE.md
- **서명**: 해당 폴더의 SIGNATURE.md에 포함된 파일 전부 Git blob 해시로 기록됨.

이 레이어(`_06_lucifer_impact`)는 `lucifer_engine`이 설치되어 있으면 그 패키지를 사용하고, 없으면 위의 `impact_estimator.py`를 사용한다.
