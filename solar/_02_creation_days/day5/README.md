...
**움직이는 생물(새·물고기)** 가 씨드·질소·포식량을 위도 간에 나르는 **transport 항**을 더하는 레이어.
...
- **창세기 관점**: *“새들은 땅 위 하늘을 날아다녀라”* — 이동하는 생물이 **정보(씨·영양)를 지구 곳곳으로 전달**하는 단계.
- **시스템 관점**: Day3 의 `BiosphereColumn` 은 **로컬 ODE**만 담당하고, band 간 결합은 없다.  
  ...
  즉 Day5 는 **Day3 biosphere 상태의 비국소 결합(non-local coupling)** 을 구현하는 레이어다.
...
- **보존 (band index)**: source/sink 가 없으면 \(\sum_i B_i\) 는 일정.  
  수치 안정성: \(dt_{\mathrm{yr}} \cdot \sum_j K_{i\to j} \le 1\) 이면 양수성이 유지된다.  
- **면적 가중 보존 (planet accounting)**  
  지구는 위도별 면적이 다르다 (\(A_i \propto \cos\phi_i\)).  
  **전지구 질량 보존**은 면적 가중 합으로 정의해야 한다:
  \[
  \sum_i A_i\, B_i = \mathrm{const}.
  \]
  따라서 transport 커널 \(K_{i\to j}\) 는 **면적 가중 질량**을 보존하도록 설계되어야 한다.  
  면적 가중이 없으면 극지 밴드가 과도한 source/sink처럼 작동해 장기 시뮬레이션에서 biosphere drift 가 발생할 수 있다.
...
> **물리 회계 5칙 (행성 통합 시 필수)**  
> (1) **Area conservation** — Transport 는 면적 가중 전지구 질량 \(\sum_i A_i B_i\) 를 보존한다.  
> (2) **Nitrogen redistribution** — Guano 는 band 간 N 재분배이며, 전지구 N 생성이 아니다.  
> (3) **CO₂ flux** — `co2_resp_yr` 은 플럭스율 [kgC m⁻² yr⁻¹]; 대기 연동 시 \(dt_{\mathrm{yr}}\) 를 **한 번만** 곱한다.  
> (4) **Migration drivers** — 이동률은 O₂, 계절성(Day4), 자원 구배(GPP/biomass)에 의존할 수 있다.  
> (5) **Transport** — Day5 는 Day3 biosphere 상태의 비국소 결합을 담당한다.
...
- **Loop F**: 새가 씨를 먼 밴드로 옮김 → pioneer 분포가 **공간적으로 완만해짐** (long-range dispersal kernel 에 대응).  
- **Loop G**: 새의 배설물(구아노)이 **N_soil** 을 보강 → Day4 `NitrogenCycle` 입력.  
  **물리적 제약**: 구아노는 다른 밴드에서 섭취한 N 의 재분배이므로 **전지구 N 생성이 아니다**.  
  Guano transport redistributes nitrogen between bands; it is not a net source. Global nitrogen inventory remains conserved.  
- **Loop H**: 물고기 포식으로 phyto 감소 → **호흡 CO₂** 증가 → Day2 대기 조성과 연결 (Day5가 Day2 atmosphere 에 직접 연결되는 첫 포트).
...
- **이동률 수식 (BirdAgent)**  
  ...
  \]
  현재 구현은 O₂ 만 사용. **행성 레이어**로 닫히려면 이동률은 최소 다음 드라이버를 고려할 수 있다 (확장 포인트, 문서 수준):  
  **Migration rates may depend on:** O₂ availability, seasonal modifier (Day4 `SeasonEngine`), resource gradient (GPP / biomass).  
- **Transport 보존**  
  `SeedTransport.step(B, dt_yr)` 은 \(\sum_i B_i\) 를 유지 (구현은 band-index 보존; 면적 가중은 §1.2 참고).  
  `step_with_source` 는 외부 source 플럭스(예: 구아노)를 더한 뒤 같은 보존 연산.  
- **FoodWeb CO₂ (단위·통합 규칙)**  
  `TrophicState.co2_resp_yr` 은 **순간 플럭스율** [kgC m⁻² yr⁻¹] 이다.  
  **Atmosphere coupling 시** \(dt_{\mathrm{yr}}\) 를 **딱 한 번만** 곱한다.  
  즉: 대기 CO₂ 가산량 = `co2_resp_yr * dt_yr` (rate × time = delta).  
  rate 와 delta 를 혼용하면 CO₂ runaway 가 발생하므로, 통합 시 반드시 이 규칙을 따른다.  
  `net_co2_flux(state, gpp)` 는 GPP 흡수와 호흡을 합산한 **플럭스율** [kgC m⁻² yr⁻¹] 로, 동일하게 한 스텝당 한 번만 \(dt_{\mathrm{yr}}\) 를 곱해 대기와 연결한다.
...
## 6. 개념 갭 및 확장 포인트 (스크린샷·철학 대조)

“정보 전달체로서의 생물”·“전 지구적 네트워크” 개념과 현재 엔진을 **냉정 대조**한 결과다.  
Loop F/G/H 기어 연결은 완료됐고, 아래 세 가지는 **옵션 API로 구현**되어 있다 (기존 시그니처 유지).

| 스크린샷·철학 개념 | 현재 구현 | 확장 API |
|--------------------|------------|-----------|
| **대형 생물** (개체수/바이오매스) | 기본은 `base_rate` 만 사용 | `seed_flux(..., biomass_by_band=)`, `guano_flux(..., biomass_by_band=)`, `predation_flux(..., biomass_by_band=)` 로 플럭스를 바이오매스에 비례 가능 |
| **하늘·바다** (서식 도메인 분리) | 기본은 동일 위도 링 | `migration_rates(land_fraction_by_band=)` · Bird 는 육상, Fish 는 해양 밴드만 활성화 (rate × land 또는 rate × (1−land)) |
| **전 지구적 장거리 이동** | 기본은 인접 밴드만 (`i±1`) | `long_range_neighbors(n_bands, max_jump)` + `BirdAgent(long_range_max_jump=2)` 로 비인접 연결 사용 가능 |

- **정합성**: “생물의 등장”이 아니라 **“행성 네트워크의 탄생”**으로 보면 현재 설계와 일치한다.  
- **용어**: “대형”보다는 **“장거리 이동 가능한 생물”** (mobility scale > ecosystem scale) 가 시스템 정의와 맞다.  
- **확장 시**: 에이전트 바이오매스·도메인 마스크·장거리 커널을 **옵션 인자**로 넣어 두었으며, 기존 시그니처는 그대로 유지한다. 통합 시 `land_fraction_by_band` 는 Day3 `surface` / 위도 밴드별 육지 비율에서 가져오면 된다.
