...
좌표계·벡터 규약:
  - 이 모듈이 사용하는 벡터 공간은 우주 공간(절대 위·아래 없음)이지만,
    계산 일관성을 위해 **+z 축을 남극 방향(geomagnetic N극)**으로 둔다.
  - 자전축(spin_axis)과 magnetic_axis는 “남극(+z) → 북극(–z)으로 떨어지는
    자력 폭포수” 프레임을 따른다.
  - 이는 사실 확정값이 아니라 **모델링 선택(좌표계 설정)**이며,
    축을 회전해도 물리 법칙 자체는 변하지 않는다.
