...
# —— 산불 피드백 (Fire Feedback) — O₂ 자연 attractor ————————————————
# 설계 목표: O₂ 35% 하드 클램프 제거 → 산불 ODE로 물리적 attractor 구현
#
# 관측 근거 (Lenton & Watson 2000; Berner 2006):
#   - O₂ < 15%: 산불 발생 거의 불가 (건조 낙엽도 점화 안 됨)
#   - O₂ = 21%: 현재 지구 (산불 정상)
#   - O₂ = 25%: 산불 발생률 2배 이상 → 식생 감소
#   - O₂ > 30%: 거대 산불 → 육상 생태계 붕괴 수준
#
# 구현: fire_sink = K_FIRE × max(0, O2 - O2_FIRE_TH)²
#   → O₂가 임계값을 넘을수록 산불 소비가 제곱으로 증가
#   → dO2/dt = GPP_O2 - Resp_O2 - fire_sink  (진짜 ODE attractor)

O2_FIRE_TH  = 0.25            # [mol/mol] 산불 임계 O₂ 분율 (25%)
                               # 25% 아래: 산불 없음 / 위: 산불 증가
K_FIRE      = 15.0            # [1/yr / (mol/mol)] 산불 강도 계수
                               # 보정 근거: O₂=30%일 때 fire_sink ≈ 0.0038/yr
                               #           ≈ 연간 O₂ 0.38% 소비 (지구 스케일 합리적)
