...
Strang splitting의 감쇠(D) 스텝을 O-U (Ornstein-Uhlenbeck) 반스텝으로 확장:

```
기존: D-S-K-R-K-S-D     (D = dissipation only)
현재: O-S-K-R-K-S-O     (O = dissipation + noise)

O(dt/2):
  v *= exp(-γ·dt/2)              ← 감쇠 (기존과 동일)
  v += σ · √(dt/2) · N(0,1)     ← 노이즈 (Wiener increment)
```

- 대칭 래핑: 시작과 끝에 독립 노이즈 → 총 분산 σ²dt
- σ=0이면 기존 D 스텝과 동일 (하위 호환)
- Euler-Maruyama 이산화 (√dt 스케일링)
