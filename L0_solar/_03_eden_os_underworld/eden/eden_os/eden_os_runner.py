...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ...
  레이어 (조/모 = Macro–Micro): L0/L1(ic) → L2(instability) → L3(FirmamentLayer) → scenario_overlay·수명. docs/PANGEA, FEYNMAN §9.
...
from L0_solar._03_eden_os_underworld.eden.creation_layers import run_creation_layers, build_scenario_overlay

try:
    from L0_solar._03_eden_os_underworld.eden.firmament import FirmamentLayer, Layer0Snapshot
    _FIRMAMENT_AVAILABLE = True
except Exception:
    _FIRMAMENT_AVAILABLE = False
...
try:
    from L0_solar._01_beginnings.joe import compute_planet_stress_and_instability
    _PLANET_DYNAMICS_AVAILABLE = True
except Exception:
    _PLANET_DYNAMICS_AVAILABLE = False
    compute_planet_stress_and_instability = None  # type: ignore

try:
    from L0_solar._02_creation_days.day4.core import EvolutionEngine
    _DEEP_ENGINE_AVAILABLE = True
except Exception:
    _DEEP_ENGINE_AVAILABLE = False
...
class EdenOSRunner:
    ...
    def __init__(
        ...
        firmament:     Optional[Any] = None,
        deep_engine:   Optional[Any] = None,
    ) -> None:
        self._world           = world
        self._rivers          = rivers
        self._kernel          = kernel
        ...
        self._firmament       = firmament  # 궁창 레이어 (없으면 scenario_overlay/layer0 미연결)
        self._deep_engine     = deep_engine  # 물리 코어(루시퍼) — hades → deep_monitor에 전달
        self._tick            = 0
    ...
    def _compute_instability(self) -> float:
        """매 틱 궁창 step(instability=...)에 넣을 불안정도 [0~1].
        solar.joe (조 엔진) 호출. L1 물 예산은 world.ic에서 구성해 전달.
        """
        if not _PLANET_DYNAMICS_AVAILABLE or compute_planet_stress_and_instability is None:
            return 0.0
        ic = self._world.ic
        ...
        _planet_stress, instability = compute_planet_stress_and_instability(ic, water_snapshot=water_snapshot)
        return instability
    ...
    def _execute_tick(self) -> TickLog:
        ...
        # ── 천지창조 레이어 스택 (solar 순서: L0/L1 → L2 조 → L3 궁창 → 루시퍼 → 하데스) ──
        instability, layer0, hades_signal = run_creation_layers(
            self._tick,
            self._world,
            self._firmament,
            self._hades,
        ...
        if self._firmament is not None and layer0 is not None:
            scenario_overlay = build_scenario_overlay(layer0, self._lineage)
            if scenario_overlay:
                self._world = make_eden_world(ic=self._world.ic, scenario_overlay=scenario_overlay)
        ...
        #         ★ 선악과 섭취 감지 → lineage.record_expulsion() 자동 실행
        #         hades_signal = 레이어 스택(run_creation_layers)에서 이미 계산됨
        # ─────────────────────────────────────────────────────────────────────
...
def make_eden_os_runner(
    ...
    """
    if _FIRMAMENT_AVAILABLE and FirmamentLayer is not None:
        fl = FirmamentLayer()
        layer0 = fl.get_layer0_snapshot()
        scenario_overlay = {
            "shield_strength": layer0.shield_strength,
            "env_load": layer0.env_load,
            "lifespan_group": "admin_line",
            "lifespan_generation": 0,
        }
        world = make_eden_world(ic=world_ic, scenario_overlay=scenario_overlay)
    else:
        fl = None
    ...
    deep_engine = None
    if _DEEP_ENGINE_AVAILABLE and EvolutionEngine is not None:
        try:
            deep_engine = EvolutionEngine()
        except Exception:
            deep_engine = None
