...
from L0_solar._06_lucifer_impact import ImpactParams, ImpactResult, estimate_impact
...
def run_scenario_combined_impulse(
    *,
    ...
    impact_params 가 주어지면 _06_lucifer_impact.estimate_impact() 로 shock_strength 를
    산출해 그 값으로 임펄스 크기를 정한다. None 이면 기존과 동일(하드코딩 0.25/0.10, 0.9/0.7).
    (D, v, ρ, θ, h) 스윕으로 "어떤 조합이 postdiluvian_ok 를 만드는가" 실험이 가능하다.
    ...
    if impact_params is not None:
        impact: ImpactResult = estimate_impact(impact_params)
        impulse_joe = 0.25 * impact.shock_strength
        impulse_joe_tail = 0.10 * impact.shock_strength
        impulse_risk_peak = 0.9 * impact.shock_strength
        impulse_risk_tail = 0.7 * impact.shock_strength
    else:
        impulse_joe = 0.25
        impulse_joe_tail = 0.10
        impulse_risk_peak = 0.9
        impulse_risk_tail = 0.7

    def joe_instability_fn(t: float) -> float:
        frac = max(0.0, min(1.0, t / years))
        base = 0.2 + 0.6 * frac  # combined_ramp 와 동일 베이스
        if abs(t - shock_time) < 0.5:
            return min(1.0, base + impulse_joe)
        if abs(t - shock_time) < 1.0:
            return min(1.0, base + impulse_joe_tail)
        return base
