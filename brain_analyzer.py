...
class BrainAnalyzer:
    ...
    def _analyze_layer1(
        ...
        if len(velocities) >= 200:
            window = min(100, len(velocities) // 2)
        ...
        result["is_equilibrium"] = (
            abs(ep_rate) < 0.1 * self.gamma * self.T / self.mass
            and analyzer.detailed_balance_violation() < 0.1
        )
    ...
    def _analyze_layer6(
        ...
        x_min = min(centers) - 3.0
        x_max = max(centers) + 3.0
        ...
        result["curvature_nontrivial"] = abs(K) > 1e-6
