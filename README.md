...
여기서 좌표·벡터 공간은 **우주 공간(절대 위·아래 없음)**이지만,
이 프로젝트에서는 **관찰자 시점에서 계산을 단순하게 하기 위해**
다음과 같이 **모델링 규약**을 둔다.

- **+z 축 = 남극 방향(geomagnetic N극이 있는 쪽)**  
- 자기장은 “남극(+z)에서 북극(–z)으로 떨어지는 폭포수”처럼 해석한다.
- Magnetosphere, 세차, Firmament 등 모든 자력·방향 관련 계산은 이 규약을 따른다.

즉, 이것은 **사실 확정 하드코딩이 아니라 수치·좌표계 설정**이며,
필요하면 전체 축을 회전시켜도 물리 법칙 자체는 변하지 않는다.
