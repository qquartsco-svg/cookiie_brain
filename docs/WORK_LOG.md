...
## 2026-02-23 — v0.4.0 폴더 구조 리팩터링

### 동기
Phase (A/B/C)와 Layer (1~6)는 역할이 다르다:
- Phase = trunk (운동방정식 구성요소, PFE에 직접 투입)
- Layer = 분석 도구 (trunk 결과를 받아 물리량 측정/해석)

같은 계층에 두면 혼동을 줄 수 있어 폴더로 분리.

### 변경 내용
- `Phase_A/`, `Phase_B/`, `Phase_C/` → `trunk/Phase_A/`, `trunk/Phase_B/`, `trunk/Phase_C/`
- `Layer_1/` ~ `Layer_6/` → `analysis/Layer_1/` ~ `analysis/Layer_6/`
- `trunk/__init__.py`, `layers/__init__.py` 생성 (역할 설명 docstring 포함)
- 모든 Python import 경로 수정 (14개 파일)
- 모든 README/문서 내 import 예시 및 파일 경로 링크 수정
- `README.md`에 v0.3→v0.4 경로 변경 대조표 추가

### 수정 파일 (총 20+개)
- `cookiie_brain_engine.py`
- `examples/*.py` (12개 verification 스크립트)
- `analysis/Layer_1/statistical_mechanics.py`
- `README.md`, `QUICK_START.md`
- `docs/FULL_CONCEPT_AND_STATUS.md`, `docs/FULL_CONCEPT_AND_STATUS_EN.md`
- `analysis/Layer_*/README.md` (6개)

> **Note**: `layers/` 대신 `analysis/`를 사용.
> PotentialFieldEngine에 `layers.py`가 이미 있어서 이름 충돌 방지.
